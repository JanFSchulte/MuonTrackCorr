...
class NtuplizerAOD : public edm::EDAnalyzer {
    public:
        ...
        explicit NtuplizerAOD(const edm::ParameterSet&);
        virtual ~NtuplizerAOD();
    ...
    private:
        ...
        const edm::EDGetTokenT<std::vector<reco::Muon>> offlineMuonToken_;
        const edm::EDGetTokenT<reco::VertexCollection> offlinePVToken_;
        const edm::EDGetTokenT<std::vector<reco::Track>> offlineTrackToken_;
        ...
        unsigned int n_reco_mu_;
        std::vector<float> reco_mu_pt_;
        std::vector<float> reco_mu_ptErr_;
        std::vector<float> reco_mu_eta_;
        std::vector<float> reco_mu_phi_;
        std::vector<float> reco_mu_e_;
        std::vector<float> reco_mu_innerPt_;
        std::vector<float> reco_mu_innerEta_;
        std::vector<float> reco_mu_innerPhi_;
 
        std::vector<int>   reco_mu_charge_;
        std::vector<int>   reco_mu_isTight_;
        std::vector<int>   reco_mu_isMedium_;
        std::vector<int>   reco_mu_isLoose_;
        std::vector<int>   reco_mu_isSoft_;
        std::vector<float>   reco_mu_trackIso_;
        std::vector<float>   reco_mu_chargedIso_;
        std::vector<float>   reco_mu_neutralIso_;
        std::vector<float>   reco_mu_photonIso_;
        std::vector<float>   reco_mu_PUIso_;
        std::vector<float>   reco_mu_dB_;
        std::vector<float>   reco_mu_edB_;
        std::vector<int>   reco_mu_validMuonHits_;
        std::vector<float>   reco_mu_normChi2_;
        std::vector<int>   reco_mu_matchedStations_;
        std::vector<int>   reco_mu_innerTrackNLayers_;
        std::vector<int>   reco_mu_innerTrackNPixHit_;
        std::vector<int>   reco_mu_isGlobal_;
        std::vector<int>   reco_mu_isPF_;
        std::vector<int>   reco_mu_isStandalone_;
        std::vector<int>   reco_mu_isTracker_;


        unsigned int n_reco_track_;
        std::vector<float> reco_track_pt_;
        std::vector<float> reco_track_ptErr_;
        std::vector<float> reco_track_eta_;
        std::vector<float> reco_track_phi_;
        std::vector<int>   reco_track_charge_;
        std::vector<int>   reco_track_isHighPurity_;
...
void NtuplizerAOD::initialize()
{
    ...
    n_reco_mu_   = 0;
    reco_mu_pt_.clear();
    reco_mu_ptErr_.clear();
    reco_mu_eta_.clear();
    reco_mu_phi_.clear();
    reco_mu_innerPt_.clear();
    reco_mu_innerEta_.clear();
    reco_mu_innerPhi_.clear();
    reco_mu_e_.clear();
    reco_mu_charge_.clear();
    reco_mu_isTight_.clear();
    reco_mu_isMedium_.clear();
    reco_mu_isLoose_.clear();
    reco_mu_isSoft_.clear();
    reco_mu_trackIso_.clear();
    reco_mu_chargedIso_.clear();
    reco_mu_neutralIso_.clear();
    reco_mu_photonIso_.clear();
    reco_mu_PUIso_.clear();
    reco_mu_dB_.clear();
    reco_mu_validMuonHits_.clear();
    reco_mu_normChi2_.clear();
    reco_mu_matchedStations_.clear();
    reco_mu_innerTrackNLayers_.clear();
    reco_mu_innerTrackNPixHit_.clear();
    reco_mu_isGlobal_.clear();
    reco_mu_isPF_.clear();
    reco_mu_isStandalone_.clear();
    reco_mu_isTracker_.clear();


    n_reco_track_ = 0;
    reco_track_pt_.clear();
    reco_track_ptErr_.clear();
    reco_track_eta_.clear();
    reco_track_phi_.clear();
    reco_track_charge_.clear();
    reco_track_isHighPurity_.clear();
...
NtuplizerAOD::NtuplizerAOD(const edm::ParameterSet& iConfig):
    ...
    offlineMuonToken_       (consumes<std::vector<reco::Muon>>(iConfig.getParameter<edm::InputTag>("offlineMuons"))),
    offlinePVToken_       (consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("offlineVertices"))),
    offlineTrackToken_(consumes<std::vector<reco::Track>>(iConfig.getParameter<edm::InputTag>("offlineTracks")))
{
...
NtuplizerAOD::~NtuplizerAOD()
{}

void NtuplizerAOD::beginJob()
{
    ...
    tree_->Branch("n_reco_mu", &n_reco_mu_);
    tree_->Branch("reco_mu_pt", &reco_mu_pt_);
    tree_->Branch("reco_mu_ptErr", &reco_mu_ptErr_);
    tree_->Branch("reco_mu_eta", &reco_mu_eta_);
    tree_->Branch("reco_mu_phi", &reco_mu_phi_);
    tree_->Branch("reco_mu_innerPt", &reco_mu_innerPt_);
    tree_->Branch("reco_mu_innerEta", &reco_mu_innerEta_);
    tree_->Branch("reco_mu_innerPhi", &reco_mu_innerPhi_);
    tree_->Branch("reco_mu_e", &reco_mu_e_);
    tree_->Branch("reco_mu_charge", &reco_mu_charge_);   
    tree_->Branch("reco_mu_isTight", &reco_mu_isTight_);   
    tree_->Branch("reco_mu_isMedium", &reco_mu_isMedium_);   
    tree_->Branch("reco_mu_isLoose", &reco_mu_isLoose_);   
    tree_->Branch("reco_mu_isSoft", &reco_mu_isSoft_);   
    tree_->Branch("reco_mu_trackIso", &reco_mu_trackIso_);   
    tree_->Branch("reco_mu_chargedIso", &reco_mu_chargedIso_);   
    tree_->Branch("reco_mu_neutralIso", &reco_mu_neutralIso_);   
    tree_->Branch("reco_mu_photonIso", &reco_mu_photonIso_);   
    tree_->Branch("reco_mu_PUIso", &reco_mu_PUIso_);   
    tree_->Branch("reco_mu_dB", &reco_mu_dB_);   
    tree_->Branch("reco_mu_edB", &reco_mu_edB_);   
    tree_->Branch("reco_mu_validMuonHits", &reco_mu_validMuonHits_);   
    tree_->Branch("reco_mu_normChi2", &reco_mu_normChi2_);   
    tree_->Branch("reco_mu_matchedStations", &reco_mu_matchedStations_);   
    tree_->Branch("reco_mu_innerTrackNLayers", &reco_mu_innerTrackNLayers_);   
    tree_->Branch("reco_mu_innerTrackNPixHit", &reco_mu_innerTrackNPixHit_);   
    tree_->Branch("reco_mu_isGlobal", &reco_mu_isGlobal_);   
    tree_->Branch("reco_mu_isPF", &reco_mu_isPF_);   
    tree_->Branch("reco_mu_isStandalone", &reco_mu_isStandalone_);   
    tree_->Branch("reco_mu_isTracker", &reco_mu_isTracker_);   

    tree_->Branch("n_reco_track", &n_reco_track_);   
    tree_->Branch("reco_track_pt", &reco_track_pt_);   
    tree_->Branch("reco_track_ptErr", &reco_track_ptErr_);   
    tree_->Branch("reco_track_eta", &reco_track_eta_);   
    tree_->Branch("reco_track_phi", &reco_track_phi_);   
    tree_->Branch("reco_track_charge", &reco_track_charge_);   
    tree_->Branch("reco_track_isHighPurity", &reco_track_isHighPurity_);   

}

void NtuplizerAOD::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{}

void NtuplizerAOD::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    // --------- in-place function
    ...
    iEvent.getByToken(offlineMuonToken_, muonsH);
    ...
    iEvent.getByToken(offlineTrackToken_, track_h);
    const reco::TrackCollection tracks = (*track_h.product());
    ...
    for (const auto& muon : muons){
        ++n_reco_mu_;
        reco_mu_pt_. push_back(muon.muonBestTrack()->pt());
        reco_mu_ptErr_. push_back(muon.muonBestTrack()->ptError());
        reco_mu_eta_. push_back(muon.eta());
        reco_mu_phi_. push_back(muon.phi());
        reco_mu_e_. push_back(muon.energy());
        reco_mu_charge_. push_back(muon.charge());

        reco_mu_isTight_. push_back(muon::isTightMuon(muon,pv));
        reco_mu_isMedium_.push_back(muon::isMediumMuon(muon));
        reco_mu_isLoose_.push_back(muon::isLooseMuon(muon));
        reco_mu_isSoft_.push_back(muon::isSoftMuon(muon,pv));
        reco_mu_trackIso_.push_back(muon.isolationR03().sumPt);
        reco_mu_chargedIso_.push_back(muon.pfIsolationR03().sumChargedHadronPt);
        reco_mu_neutralIso_.push_back(muon.pfIsolationR03().sumNeutralHadronEt);
        reco_mu_photonIso_.push_back(muon.pfIsolationR03().sumPhotonEt);
        reco_mu_PUIso_.push_back(muon.pfIsolationR03().sumPUPt);
        if (muon.globalTrack().isNonnull()){
            reco_mu_validMuonHits_.push_back(muon.globalTrack()->hitPattern().numberOfValidMuonHits());
            reco_mu_normChi2_.push_back(muon.globalTrack()->normalizedChi2());
        }
        else{
            reco_mu_validMuonHits_.push_back(0);
            reco_mu_normChi2_.push_back(-999.);
        }
        reco_mu_matchedStations_.push_back(muon.numberOfMatchedStations());
        if (!muon.innerTrack().isNull()){
            reco_mu_innerTrackNLayers_.push_back(muon.innerTrack()->hitPattern().trackerLayersWithMeasurement());
            reco_mu_innerTrackNPixHit_.push_back(muon.innerTrack()->hitPattern().numberOfValidPixelHits());
            reco_mu_dB_.push_back(muon.innerTrack()->dxy(pv.position()));
            reco_mu_edB_.push_back(-999.);
            reco_mu_innerPt_.push_back(muon.innerTrack()->pt());
            reco_mu_innerEta_.push_back(muon.innerTrack()->eta());
            reco_mu_innerPhi_.push_back(muon.innerTrack()->phi());
        }
        else{
            reco_mu_innerTrackNLayers_.push_back(0);
            reco_mu_innerTrackNPixHit_.push_back(0);
            reco_mu_dB_.push_back(-999.);
            reco_mu_edB_.push_back(-999.);
        }
        reco_mu_isGlobal_.push_back(muon.isGlobalMuon());
        reco_mu_isPF_.push_back(muon.isPFMuon());
        reco_mu_isStandalone_.push_back(muon.isStandAloneMuon());
        reco_mu_isTracker_.push_back(muon.isTrackerMuon());
    }

    for (const auto& track : tracks){

        ++n_reco_track_;
        reco_track_pt_.push_back(track.pt());
        reco_track_ptErr_.push_back(track.ptError());
        reco_track_eta_.push_back(track.eta());
        reco_track_phi_.push_back(track.phi());
        reco_track_charge_.push_back(track.charge());
        reco_track_isHighPurity_.push_back(track.quality(Track::highPurity));
    }
...
void NtuplizerAOD::endJob()
{}

void NtuplizerAOD::endRun(edm::Run const& iRun, edm::EventSetup const& iSetup)
{}


#include <FWCore/Framework/interface/MakerMacros.h>
DEFINE_FWK_MODULE(NtuplizerAOD);
